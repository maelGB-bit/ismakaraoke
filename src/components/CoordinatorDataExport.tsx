...
interface CoordinatorDataExportProps {
  instanceId: string;
  instanceName: string;
  instanceCode: string;
}

export function CoordinatorDataExport({ instanceId, instanceName, instanceCode }: CoordinatorDataExportProps) {
  ...
  const downloadParticipants = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `participantes_${instanceCode}_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const downloadRanking = async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `ranking_${instanceCode}_${new Date().toISOString().split('T')[0]}.csv`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Exporte os dados da instância <strong>{instanceName}</strong> ({instanceCode})
          </DialogDescription>
        </DialogHeader>
