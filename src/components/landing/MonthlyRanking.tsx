...
export function MonthlyRanking() {
  ...
  useEffect(() => {
    const fetchRankings = async () => {
      try {
        // Query performances directly with the same logic as the view
        ...
        // Calculate global score and sort
        const ranked = (data || [])
          .map(entry => ({
            id: entry.id,
            cantor: entry.cantor,
  ...
  if (loading) {
    return (
      <section className="py-16 bg-landing-dark">
  ...
  return (
    <section className="py-16 bg-gradient-to-b from-landing-dark to-landing-brown/20">
